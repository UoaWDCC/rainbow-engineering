...
export default function ExecCard({ name, role, imageSrc, description }: ExecCardProps) {
  return (
    <div className="flex flex-col items-center text-center bg-[#E2D6F6] p-6 rounded-2xl">
      ...
      />
      <p className="text-sm md:text-base text-[#5f249f] font-semibold font-[Montserrat]">{name}</p>
      <p className="text-xs md:text-sm text-[#5f249f] font-semibold font-[Montserrat] mb-1">{role}</p>
